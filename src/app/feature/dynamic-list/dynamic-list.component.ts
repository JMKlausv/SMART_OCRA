...
import { Column, CommandModel, DataResult, DataStateChangeEventArgs, EditSettingsModel, FilterSettingsModel, GridComponent, GridModel, GroupSettingsModel, IRow, PageSettingsModel, SearchSettingsModel, SelectionSettingsModel, SortEventArgs } from '@syncfusion/ej2-angular-grids';
...
load(){
  ...
  initializeCommands() {
    if (this.incomingCommand) {
      console.log("incomingCommand : ",this.incomingCommand)
      if (this.viewPrivilege && (this.incomingCommand?.page=='operation')) {
  ...
  onDetail(args: any): void {
    ...
    this.editRecord.emit(data1);
    let formId = this.listType.pageName == 'assign_goods' ? data1.operation.id : data1.id;
    this.listType ? this.navigate(this.listType.formPath + '/' + formId + '/detail') : null;
  }
  ...
  onDelete(args: Event): void {
    ...
    const data1: any = rowObj.data;
    console.log("this.crudService.deleting",this.crudService.deleting)
